...
class Config(object):
    ...
    API_ID = int(os.environ.get("API_ID",  "22656556"))
    API_HASH = os.environ.get("API_HASH", "dc4ec57887041a067ca3f6d61d1e90cd")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '5802249521').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002175175294').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002229745506")
